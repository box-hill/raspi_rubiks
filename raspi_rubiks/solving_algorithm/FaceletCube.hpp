...
public:
    // Face names
    enum Face { U, D, L, R, F, B };  //enum specifies 0,1,2,3... ( U = 0, D = 1...)
    ...
    // Set the cube markings for a given face (Giving each facelet a number)
    void SetFaceMarkings(int face, string faceMarkings); //setting up the facelet indices (basically giving a number to each facelet, starting from 0 to 53)

    // Validate markings, permutation, and parity
    int Validate(Cube &cube); //I'll have a look later on how to validate markings

    // Convert face name to enumeration offset
    int FaceNameToOffset(char faceName);    //used to check if valid (defined in FaceletCube.cpp)
...
private:
    ...
    enum Corner {
        URF = FacesToCorner(U,R,F), // U = 0, D = 1 as defined above.
