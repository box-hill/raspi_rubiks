...
int Solver::Solve(RubiksCube& scrambledCube)
{
    ...
    do
    {
        ...
        iteration++;
        //cout<< "test " << solutionLength1 << endl;
    } while (iteration < 6);
...
void Solver::PrintSolution(void) {
    ...
    if(solutionLength1 + solutionLength2 <= 22){
        string sol;
        for(int i = 0; i < solutionLength1; i++){
            sol += Cube::NameOfMove(TranslateMove(solutionMoves1[i], solutionPowers1[i], 0)) + " ";
        }

        for(int i = 0; i < solutionLength2; i++){
            sol += Cube::NameOfMove(TranslateMove(solutionMoves2[i], solutionPowers2[i], 1)) + " ";
        }
        ofstream myfile("solution.txt", ios::trunc);
        myfile << sol;
        myfile.close();
...
string Solver::SaveSolution(void) {
    string sol;
    for(int i = 0; i < solutionLength1; i++){
        sol += Cube::NameOfMove(TranslateMove(solutionMoves1[i], solutionPowers1[i], 0)) + " ";
    }
