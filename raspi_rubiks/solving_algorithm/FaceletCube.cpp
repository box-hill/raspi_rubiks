...
// Set the cube markings for a given face (54 facelets, first facelet is 0, last facelet is 53)
void FaceletCube::SetFaceMarkings(int face, string markings) {
    int facelet;
    for (facelet = 0; facelet < FACELETS_PER_FACE; facelet++) //FACELETS_PER_FACE defined as 9
...
// Dump cube state
void FaceletCube::Dump() {  //this just prints to the console (Vector.hpp)
...
int FaceletCube::FaceNameToOffset(char faceName) {
    ...
    if (!(name = strchr(faceNames, faceName)))  //strchr finds the character in the string and return its position
        return -1;
